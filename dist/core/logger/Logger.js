...
import { IndicatorManager, wrapIndicatorManager, } from './components/indicators/IndicatorManager.js';
...
class Logger {
    ...
    getIndicatorStr(level, indicate) {
        if (indicate)
            return this.i.getIndicator(indicate).str;
    ...
    createGroupHead(headDetail) {
        const { command, indicate, color, label = this.defaults.label, level = this.defaults.level, } = headDetail;
        return {
            level,
    ...
    log(detail) {
        ...
        if (detail.body)
            this.writeGroupBody(detail.body, msgColor, level);
        this.console.groupEnd();
        this.console.log();
    }
}
