...
import { LOG_LEVELS, Logger, type LogDetail } from '../../logger/Logger.js';
 ...
 */
const executedDetail = (result: CommandResult) => {
  ...
  const body: [{ key: string; value: string; color?: string }] = [
    { key: 'time', value: getTime() },
  ];
  if (stderr)
    body.push({ key: 'stderr', value: stderr, color: commandObj.errorColor ?? '#ff3636' });
  if (stdout) body.push({ key: 'stdout', value: stdout });
  return {
    head: {
      command: commandStr,
      label,
 ...
 */
const createdDetail = (cmd: Command): LogDetail => ({
  head: {
 ...
 */
const updatedDetail = (oldCmd: Command, newCmd: Command): LogDetail => ({
  head: {
 ...
 */
const deletedDetail = (cmdStr: string): LogDetail => ({
  head: {
